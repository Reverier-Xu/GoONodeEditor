...
QRectF NodeGeometry::boundingRect() const {
    ...
    double addon = 6 * nodeStyle.ConnectionPointDiameter;

    return QRectF(0 - addon, 0 - addon / 2, _width + 2 * addon,
                  _height + addon);
}
...
QPointF NodeGeometry::portScenePosition(PortIndex index, PortType portType,
    ...
    switch (portType) {
        case PortType::Out: {
            double x =
                (double)_width + (nodeStyle.ConnectionPointDiameter * 2 + 3);
        ...
        case PortType::In: {
            double x = 0.0 - (nodeStyle.ConnectionPointDiameter * 2 + 3);
