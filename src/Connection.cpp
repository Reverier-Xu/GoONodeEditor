...
void Connection::setGraphicsObject(
    ...
    // 仅当创建ConnectionGraphicsObject的时候此函数才会被调用.
    // 创建连接对象之后连接对象的两个端点位置均在(0, 0),
    // 整个连接对象的位置也在GraphicsView的(0, 0)处.
    // 只要把整个对象的两个端点正确定位到Node对象对应的接口上,
    // 此连接对象就能被正确定位.
...
void Connection::transmitData(std::shared_ptr<NodeData> nodeData) const {
    if (_inNode) {
        if (_converter) {
            nodeData = _converter(nodeData);
        }

        _inNode->transmitData(nodeData, _inPortIndex);
    }
}

void Connection::transmitEmptyData() const {
    std::shared_ptr<NodeData> emptyData;

    transmitData(emptyData);
}
