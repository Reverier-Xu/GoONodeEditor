    ...
    for (PortType portType : {PortType::Out, PortType::In}) {
        ...
        for (unsigned int i = 0; i < n; ++i) {
            ...
            if (connectionStyle.useDataDefinedColors()) {
                painter->setBrush(
                    QtNodes::ConnectionStyle::normalColor(dataType.id));
            } else {
                painter->setBrush(nodeStyle.ConnectionPointColor);
            }
            QPointF pn = p;
            if (portType == PortType::In)
                pn.rx() += 3.5 * reducedDiameter;
            else if (portType == PortType::Out)
                pn.rx() -= 3.5 * reducedDiameter;
            painter->drawEllipse(p, reducedDiameter, reducedDiameter);
            painter->setBrush(Qt::NoBrush);
            QPen pen;
            pen.setColor(QColor(180, 180, 180, 200));
            pen.setWidth(2);
            pen.setStyle(Qt::DashLine);
            painter->setPen(pen);
            painter->drawEllipse(p, reducedDiameter * r + 2,
                                 reducedDiameter * r + 2);
            painter->setPen(Qt::NoPen);
            painter->setBrush(QBrush(QColor("grey")));
            painter->drawEllipse(pn, reducedDiameter / 2, reducedDiameter / 2);
            painter->setBrush(Qt::NoBrush);
        }
    };
}
    ...
    for (PortType portType : {PortType::Out, PortType::In}) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (!state.getEntries(portType)[i].empty()) {
                ...
                painter->setPen(Qt::NoPen);
                painter->setBrush(QBrush(QColor(20, 255, 20)));
                QPointF pn = p;
                if (portType == PortType::In)
                    pn.rx() += 2.1 * diameter;
                else if (portType == PortType::Out)
                    pn.rx() -= 2.1 * diameter;
                painter->drawEllipse(pn, diameter * 0.3, diameter * 0.3);
                painter->setBrush(Qt::NoBrush);
            }
        }
    }
